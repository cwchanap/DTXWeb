...
- Native logic lives in Rust under `packages/dtx-desktop/src-tauri/src/`: `api.rs` (API calls), `auth.rs` (Device Authorization, session validation, and external URL handling), `filesystem.rs` (workspace file access), `songs.rs`, `updater.rs`, `models.rs`, `error.rs`. Commands are exposed to the Svelte frontend via Tauri's IPC.
