    ...
    A: GoogleDriveApi + ?Sized,
{
        ...
        .filter(|pending| pending.user_id == user_id)
    {
        ...
        // the original file ID cannot be reconstructed, so we cannot guard
        // the patch — fail closed by compensating (delete the orphaned
        // Drive file and remove the pending binding) rather than issuing an
        // unconditional patch that could overwrite a newer binding
