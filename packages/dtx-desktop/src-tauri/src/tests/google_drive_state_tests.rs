...
fn drive_metadata_error_code_strings_are_stable_renderer_contract() {
    ...
    assert_eq!(
        DriveMetadataError::BindingMismatch.code(),
...
/// A scripted `AuthorizedDriveRequest` that pops queued results so a test can
/// drive the token-expiry retry ladder inside `execute_authorized_request`.
struct ScriptedAuthorizedRequest {
    results: std::sync::Mutex<
        std::collections::VecDeque<std::result::Result<String, AuthorizedDriveRequestError>>,
    >,
}

impl ScriptedAuthorizedRequest {
    fn new(results: Vec<std::result::Result<String, AuthorizedDriveRequestError>>) -> Self {
        Self {
...
#[async_trait::async_trait]
impl AuthorizedDriveRequest<String> for ScriptedAuthorizedRequest {
    async fn execute(
        &self,
        _access_token: &str,
    ) -> std::result::Result<String, AuthorizedDriveRequestError> {
        self.results
            .lock()
            .unwrap()
            .pop_front()
            .expect("a scripted result must be queued for every execute call")
    }
}

#[tokio::test]
async fn execute_authorized_request_surfaces_the_request_error_after_a_token_expiry_retry() {
    let state = state_with_provider(Arc::new(QueuedRefreshProvider::new(vec![Ok(
        token_response_with("fresh-token", 3600, Some(GOOGLE_DRIVE_FILE_SCOPE)),
    )])));
    // Cache a valid token so the first access-token lookup does not refresh;
    // the refresh fires only after the first execute reports TokenExpired.
    state.cache_access_token("user-42", "initial-token").await;
    let request = ScriptedAuthorizedRequest::new(vec![
...
#[tokio::test]
async fn execute_authorized_request_surfaces_invalid_response_when_the_refreshed_token_expires_again(
) {
    let state = state_with_provider(Arc::new(QueuedRefreshProvider::new(vec![Ok(
        token_response_with("fresh-token", 3600, Some(GOOGLE_DRIVE_FILE_SCOPE)),
    )])));
    ...
    // The retried token must be evicted after the second expiry.
    assert_eq!(state.cached_access_token("user-42").await, None);
}
...
async fn api_drive_metadata_client_fetch_owner_simfile_errors_without_a_session() {
    ...
    let auth = AuthState::default();
    // Without a session or API configuration the call must fail closed without
    // contacting the network, exercising the adapter delegation body.
    assert!(client.fetch_owner_simfile(&auth, "sim-1").await.is_err());
}
