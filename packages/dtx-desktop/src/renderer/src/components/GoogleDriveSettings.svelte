...
<section class="border-hairline mt-6 space-y-3 border-t pt-6" aria-labelledby="google-drive-title">
	...
	{#if $googleDriveStore.connection?.connected}
		...
		{/if}
	{:else}
		<p class="text-dim text-sm">{$_('googleDrive.disconnectWarning')}</p>
	{/if}
	...
	{#if $googleDriveStore.connection?.connected}
		...
		<p class="text-dim text-sm">{$_('googleDrive.folderDoesNotMoveFiles')}</p>
	{/if}

	<div class="flex flex-wrap gap-2">
		{#if !$googleDriveStore.connection?.connected || $googleDriveStore.connection?.requiresReconnect}
			<button
