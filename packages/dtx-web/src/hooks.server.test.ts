...
describe('hooks.server.ts - Bearer token authentication', () => {
	...
	it('allows unauthenticated GET /api/simFile/listFiles/:simfileID without bearer token', async () => {
		const mockGetUser = vi.fn();
		const mockGetSession = vi.fn().mockResolvedValue({
			data: { session: null }
		});

		const event = createMockEvent('http://localhost:5173/api/simFile/listFiles/123', {});
		...
		// Should allow the request through to the route handler
		expect(result).toBe(resolveResponse);
		expect(resolve).toHaveBeenCalledTimes(1);
		expect(mockGetUser).not.toHaveBeenCalled();
		expect(event.locals.user).toBeNull();
	...
	it('authenticates bearer token for GET /api/simFile/listFiles/:simfileID requests', async () => {
		const mockGetUser = vi.fn().mockResolvedValue({
			data: { user: { id: 'test-user-id', email: 'test@example.com' } },
			error: null
		});
		...
		const event = createMockEvent('http://localhost:5173/api/simFile/listFiles/456', {
			Authorization: `Bearer ${testToken}`
		});
